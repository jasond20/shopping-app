...
public class DatabaseDriverAndroid extends SQLiteOpenHelper {
  ...
  protected boolean updateAccountLine(int accountId, int itemId,
                                             int quantity) {
    String sql = "UPDATE ACCOUNTSUMMARY SET QUANTITY = ? WHERE ACCTID = ? AND ITEMID = ?;";
    SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
    ContentValues contentValues = new ContentValues();
    contentValues.put("QUANTITY", quantity);
    boolean result = sqLiteDatabase.update("ACCOUNTSUMMARY", contentValues,
            "ID=? AND ITEMID=?", new String[] {String.valueOf(itemId)}) > 0;
    sqLiteDatabase.close();
    return result;
  }
